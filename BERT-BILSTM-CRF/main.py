...
class Trainer:
    def __init__(self,
                 output_dir=None,  # 保存模型的目录
                 model=None,       # 模型实例
                 train_loader=None,# 数据加载器
                 save_step=500,    # 保存模型的间隔步骤数
                 dev_loader=None,
                 test_loader=None,
                 optimizer=None,   # 优化器
                 schedule=None,    # 学习率调度器
                 epochs=1,         # 训练轮数
                 device="cuda:0",     # 设备（CPU或GPU）
                 id2label=None):   # 标签ID到标签名称的映射
...
def main(data_name):
    ...
    with open(os.path.join(args.data_path, "train.txt"), "r", encoding="utf-8") as fp1:
        train_data = fp1.read().split("\n")
    train_data = [json.loads(d) for d in train_data]


    with open(os.path.join(args.data_path, "dev.txt"), "r", encoding="utf-8") as fp2:
        dev_data = fp2.read().split("\n")
